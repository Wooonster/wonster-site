...
*步骤 2 — SFT 消融（第 4 节）：*
- 构建两个 800 样本数据集：$\mathcal{D}_{\mathrm{ug}}$（无条件，~12k token/响应，高认知标记）和 $\mathcal{D}_{\mathrm{sg}}$（解答引导，~2k token/响应，认知标记被抑制）
...
| 基准 | 基础模型 | SFT on $\mathcal{D}_{\mathrm{ug}}$（无条件） | SFT on $\mathcal{D}_{\mathrm{sg}}$（解答引导） | Δ（引导 vs. 基础） |
|---|---|---|---|---|
...
**消融实验分析：**
关键消融（Table 2）：仅在正确的解答引导轨迹（$\mathcal{D}_{\mathrm{sg}}$）上训练导致灾难性退化，尽管训练错误率为 0%。这直接将认知抑制隔离为原因——而非答案不正确。固定教师（EMA=0.0）通过避免反馈回路放大，持续优于动态教师（EMA=0.05）[paper]。top-k 变化（100 vs. 256）无显著影响 [paper]。
