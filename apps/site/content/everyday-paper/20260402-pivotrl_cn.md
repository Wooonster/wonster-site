...
*步骤 1 — 离线 Pivot 识别：*
- 从专家 SFT 轨迹中提取所有助手轮次 → 候选集 D_cand
- 对每个候选状态 s，从参考策略 π₀ 采样 K 次局部 rollout
- 计算 μ̂(s) = 功能奖励均值，σ̂²(s) = 奖励方差
- 保留满足以下条件的轮次：σ̂²(s) > 0 且 μ̂(s) < λ_diff（难度阈值）
- 结果：D_pivot ⊂ D_cand，仅包含混合结果、具有挑战性的状态
...
- **定理 3.2**：自然梯度范数 = Var(r(s,a)) / β² → 奖励方差直接决定学习信号强度。高方差 pivot 最大化梯度幅度。
- **定理 3.3**：功能奖励优化在 M(s) 和 M(s)ᶜ 内均保留参考策略的动作排序 → 解释 OOD 保留：与当前任务无关的动作保持其参考策略比例不变。
...
**复杂度分析：**
- Pivot 识别：O(|D_cand| × K × rollout_cost)——可并行化的离线操作 [inferred]
- RL 训练：在 |D_pivot| ≪ |D_cand| 的轮次上操作；论文声明在编程任务上比 E2E RL 少约 4 倍 rollout [paper]
...
| 配置 | 准确率 |
|---|---|
| 完整 PivotRL（D_pivot + 功能奖励） | 63.81 [paper] |
| 无 Pivot 过滤（D_cand + 功能奖励） | 59.68 [paper] |
| 无功能奖励（D_cand + 严格匹配） | 57.34 [paper] |
| SFT 基线 | 58.44 [paper] |
...
- 难度阈值 λ_diff 需要按领域调优；论文给出了值但未描述敏感性。
